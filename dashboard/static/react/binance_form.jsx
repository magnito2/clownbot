...
class BinanceForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
        order_type:'',
  ...
  handleChange = name => (event) => {
    ...
    handleSubmit(e) {
        ...
        const { order_type, symbol, side, price, quantity, stop_loss, trailing_stop, take_profit, rebuy_targets } = this.state;

        const requestOptions = {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                order_type, symbol, side, price, quantity, stop_loss, trailing_stop, take_profit, rebuy_targets
            })
        };

        return fetch(`/api/binance/settings`, requestOptions)
...
const domContainer = document.querySelector('#binance_form');
ReactDOM.render(<BinanceForm/>, domContainer);