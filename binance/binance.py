...
_URLS = {
    ...
    "ping": __v1_url("ping"),
    "time": __v1_url("time"),
    "exchangeInfo": __v1_url("exchangeInfo"),
...
def _geturl_json(url, query_params=None, sign=False, method="GET", api_key=None, api_secret_key=None):
    ...
    if _URLS['exchangeInfo'] in url:
        _log(f"[+] Got exchange info {url} and sign is {sign} and query params is {query_params}")

    if not type(query_params) == dict:
        query_params = {}
    ...
    if query_params is not None:
        ...
        if _URLS['exchangeInfo'] in url:
            _log(f"Got exchange info {url} and sign is {sign}")
...
def exchange_info():
    """
    returns a dict of exchange info, look for what you want. ;/
    :return:
    """
    data = _geturl_json(_URLS["exchangeInfo"])
    return data
...
class Account:
    ...
    def get_info_backup(self):
        """ for some annoying unknown reason, sometimes the above function fails, this is backup"""
        ...
        sig = hmac.new(self.__api_secret_key.encode(), signature_parameter_string.encode(), hashlib.sha256).hexdigest()
